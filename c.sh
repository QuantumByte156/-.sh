...
RM-rf/storage/emulated/0/Android/data/com.Tencent.TMGP.DFM/files/UE4Game/UAGame/Manifest_UFSFiles_Android。文本

echo-e"\033[32m正在遍历内部存储\033[0m"

# 
log_keywords=("Logs""Centauri""TGPA""缓存""日志""调试""错误日志""游戏日志")
...
文件夹名称=$(基本名称“$dir”)
is_folder_log=false
对于“${log_keywords[@]}”中的关键字；执行
如果[[$folder_name==*"$keyword"*]]；则
is_folder_log=true
打破
Fi
已完成
    ...
    # 
如果$是文件夹日志；则
echo-e"\033[32m该文件夹可能是存储日志的文件夹，继续遍历：$目录\033[0m"
log_folders+=("$dir")
Fi

本地total_files=$(查找“$dir”-最大深度1-type f|wc-l)
本地processed_files=0
对于"$dir"/*；do中的文件
如果[[-f$file]]；则
文件名=$(基本名称"$文件")"$文件")
is_log=假的
对于"${log_keywords[@]}”中的关键字；执行"${log_keywords[@]}”中的关键字；执行
如果[[如果[[==*如果[[如果[[==*==*"$关$关
is_log=正确
打破
Fi
已完成
如果
echo-e"echo-eecho-e33[3500万$file被判断为可能是残留日志文件\033[0米"
日志文件(_F)+=("$file")
其他
echo-e"echo-eecho-e"\033[3100万$file不太可能是残留日志文件\033[0米"33[3100万$file不太可能是残留日志文件\033[0m"
Fi
((processed_files++))
进度=$((processed_files*100/total_files))
回声-neecho-ne"${progress}回波-ne"进度：[
Elif[[-dElif[[-d]]；然后
echo-eecho-e"$文echo-e""\033[32m正在遍历子文件夹：[0m$文
process_directoryprocess_directory”进程目录进程目录""
Fi
已完成
echo-e“echo-eecho-e”
}
