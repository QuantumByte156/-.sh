...
parallel_process_tools() {
    ...
    for ((i=$start_from; i<$total; i++)); do
        local tool="${tools[i]}"
日志"PROG" "处理工具 $((i+1))/$总计: $工具..."
        
        clone_or_update_tool "$工具" &
        pids+=($!)
        running=$((running+1))
        
        # 控制并行数
        while [ $running -ge $MAX_PARALLEL ]; 做
            wait -n 2>/dev/null
            running=$((running-1))
        已完成
    已完成
...
当地的未知的=0
当地的success_tools=()
    local failed tools(_T)=()
    
日志"信息" "----------------------------------------"
日志"信息" "序号  工具名称        状态        最后更新时间"
日志"信息" "----------------------------------------"
    
    local 我=1
    为 tool 在……内 "${！tools[@]}"; 做
当地的状态=$(jq-r".$工具.status" "$CONFIG_FILE"2>/dev/null||"未知")
当地的更新=$(jq-r".$工具.Last_updated" "$CONFIG_FILE"2>/dev/null||"从未更新")
        
日志"INFO" "$i. ${tool:0：15}  ${status:0：7}  $已更新"
        
        如果 [ "$status"="已克隆"] || ["$status"="已更新" ]; 然后
            成功=$((成功+1))
success_tools+=("$工具")
        Elif [ "$status"="失败" ]; 然后
            失败=$((失败+1))
...
日志"信息”"----------------------------------------"
日志"日志"信息""总结：成功$成功，失败$失败，未知$未知的，成功率：$(echo"比例=2；($success*100)/$总计"|bc-l)%"""总结：成功$成功，失败$失败，未知$未知的，成功率：$(回波"比例=2；($success*100)/$总计"|bc-l)%"
    
如果[$失败->0]；则
日志"警告""失败工具列表：${failed_tools[*]}"
日志"信息""建议对失败工具执行：./security_tools.sh-retry-failed"
Fi
}
...
# 欢迎界面
echo-e"\n${BLUE}===============================================${NC}"
echo-e"${BLUE}网络安全工具智能管理系统v4.2${NC}"
